...
pub struct WindowsInternalConf;

impl PacketSourceConf for WindowsInternalConf {
    type Task = WindowsInternalTask;
    type Data = UnboundedSender<InterceptConf>;

    fn name(&self) -> &'static str {
        "Windows internal proxy"
    }

    async fn build(
        self,
        _transport_events_tx: Sender<TransportEvent>,
...
pub struct WindowsInternalTask {
    transport_commands_rx: UnboundedReceiver<TransportCommand>,
    conf_rx: UnboundedReceiver<InterceptConf>,
    shutdown: shutdown::Receiver,
}

impl PacketSourceTask for WindowsInternalTask {
    async fn run(self) -> Result<()> {
        let WindowsInternalTask {
            mut transport_commands_rx,
            mut conf_rx,
            mut shutdown,
            ..
        } = self;
        ...
        // Spawn threads for packet capture
        ...
        tokio::spawn(async move {
            if let Err(e) = handle_ipc_internal(ipc_rx, event_tx_clone).await {
            ...
            tokio::select! {
                ...
                conf = conf_rx.recv() => {
                    if let Some(new_conf) = conf {
                        ...
                        if let Ok(network_table) = crate::windows::network::network_table() {
                ...
                command = transport_commands_rx.recv() => {
                    if let Some(cmd) = command {
                        if let Err(e) = handle_transport_command(cmd, &inject_handle).await {
                            error!("Error handling transport command: {}", e);
                        }
                    } else {
                        break;
                    }
            ...
            match connections.get_mut(&packet.connection_id()) {
                ...
                None => {
                    if address.outbound() {
                        ...
                        let action = if let Some(proc_info) =
...
async fn handle_transport_command(
    ...
    match command {
        TransportCommand::WriteData(_, data) => {
            // Convert transport command to packet injection
            let mut address = unsafe { WinDivertAddress::<NetworkLayer>::new() };
            address.set_outbound(true);
            address.set_ip_checksum(false);
            address.set_tcp_checksum(false);
            ...
            let packet = match InternetPacket::try_from(data) {
                Ok(p) => p,
                Err(e) => {
                    debug!("Error parsing packet from transport command: {:?}", e);
                    return Ok(());
                }
            };
        ...
        _ => {
            // Handle other transport commands as needed
        }
    }
    Ok(())
}

async fn handle_ipc_internal(
    mut ipc_rx: UnboundedReceiver<ipc::PacketWithMeta>,
    event_tx: UnboundedSender<Event>,
) -> Result<()> {
    while let Some(packet) = ipc_rx.recv().await {
        // Convert PacketWithMeta to Event::Ipc
        let message = ipc::from_proxy::Message::Packet(ipc::Packet {
            data: packet.data,
        });
        if let Err(e) = event_tx.send(Event::Ipc(message)) {
            error!("Failed to send IPC event: {}", e);
            break;
        }
    }
    Ok(())
